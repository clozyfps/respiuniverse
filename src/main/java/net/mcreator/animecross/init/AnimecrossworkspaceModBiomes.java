...
public class AnimecrossworkspaceModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, AnimecrossworkspaceMod.MODID);
	public static final RegistryObject<Biome> SHINOBI_FOREST = REGISTRY.register("shinobi_forest", () -> ShinobiForestBiome.createBiome());
	public static final RegistryObject<Biome> VOLCANIC_GRAND_MAGIC_ZONE = REGISTRY.register("volcanic_grand_magic_zone",
			() -> VolcanicGrandMagicZoneBiome.createBiome());
	public static final RegistryObject<Biome> PURGATORY_MAIN_BIOME = REGISTRY.register("purgatory_main_biome",
			() -> PurgatoryMainBiomeBiome.createBiome());

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			ShinobiForestBiome.init();
			VolcanicGrandMagicZoneBiome.init();
			PurgatoryMainBiomeBiome.init();
		});
	}
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(ShinobiForestBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, SHINOBI_FOREST.getId()))));
						parameters.add(new Pair<>(VolcanicGrandMagicZoneBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, VOLCANIC_GRAND_MAGIC_ZONE.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, SHINOBI_FOREST.getId()),
									Blocks.GRASS_BLOCK.defaultBlockState(), Blocks.DIRT.defaultBlockState(), Blocks.DIRT.defaultBlockState()));
							surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, VOLCANIC_GRAND_MAGIC_ZONE.getId()),
									Blocks.BLACKSTONE.defaultBlockState(), Blocks.LAVA.defaultBlockState(), Blocks.LAVA.defaultBlockState()));
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.NETHER_LOCATION)) {
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							List<SurfaceRules.RuleSource> surfaceRules = new ArrayList<>(sequenceRuleSource.sequence());
							surfaceRules.add(2, anySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, VOLCANIC_GRAND_MAGIC_ZONE.getId()),
									Blocks.BLACKSTONE.defaultBlockState(), Blocks.LAVA.defaultBlockState(), Blocks.LAVA.defaultBlockState()));
							NoiseGeneratorSettings moddedNoiseGeneratorSettings = new NoiseGeneratorSettings(noiseGeneratorSettings.noiseSettings(),
									noiseGeneratorSettings.defaultBlock(), noiseGeneratorSettings.defaultFluid(),
									noiseGeneratorSettings.noiseRouter(),
