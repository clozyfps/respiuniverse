...
import net.mcreator.animecross.procedures.GarouEntityDiesProcedure;
import net.mcreator.animecross.init.AnimecrossworkspaceModEntities;

@Mod.EventBusSubscriber
public class GarouEntity extends Monster implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.AMBIENT).add(new MobSpawnSettings.SpawnerData(AnimecrossworkspaceModEntities.GAROU.get(), 1, 1, 1));
	}

	public GarouEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AnimecrossworkspaceModEntities.GAROU.get(), world);
	}

	public GarouEntity(EntityType<GarouEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		GarouEntityDiesProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
	}

	public static void init() {
		SpawnPlacements.register(AnimecrossworkspaceModEntities.GAROU.get(), SpawnPlacements.Type.NO_RESTRICTIONS,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(GarouEntity.RemovalReason.KILLED);
		}
	}
