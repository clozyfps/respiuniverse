...
import net.mcreator.animecross.procedures.MarineCaptainRightClickedOnEntityProcedure;
import net.mcreator.animecross.init.AnimecrossworkspaceModEntities;

@Mod.EventBusSubscriber
public class MarineCaptainEntity extends Monster implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.AMBIENT)
				.add(new MobSpawnSettings.SpawnerData(AnimecrossworkspaceModEntities.MARINE_CAPTAIN.get(), 5, 1, 1));
	}

	public MarineCaptainEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AnimecrossworkspaceModEntities.MARINE_CAPTAIN.get(), world);
	}

	public MarineCaptainEntity(EntityType<MarineCaptainEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Marine Captain "));
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		MarineCaptainRightClickedOnEntityProcedure.execute(world, x, y, z, sourceentity);
		return retval;
	}

	public static void init() {
		SpawnPlacements.register(AnimecrossworkspaceModEntities.MARINE_CAPTAIN.get(), SpawnPlacements.Type.NO_RESTRICTIONS,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(MarineCaptainEntity.RemovalReason.KILLED);
		}
	}
