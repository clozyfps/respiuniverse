...
public class FireForceCaptainRightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity sourceentity) {
		...
		if ((sourceentity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) instanceof LivingEntity) {
			...
			if ((sourceentity.getCapability(AnimecrossworkspaceModVariables.PLAYER_VARIABLES_CAPABILITY, null)
					.orElse(new AnimecrossworkspaceModVariables.PlayerVariables())).fireforce == false
					|| (sourceentity.getCapability(AnimecrossworkspaceModVariables.PLAYER_VARIABLES_CAPABILITY, null)
							.orElse(new AnimecrossworkspaceModVariables.PlayerVariables())).BecomingFireForce == false) {
				{
					if (sourceentity instanceof ServerPlayer _ent) {
						...
						NetworkHooks.openGui((ServerPlayer) _ent, new MenuProvider() {
							...
							public Component getDisplayName() {
								return new TextComponent("TALKGUIFIREFORCECAPTAIN");
							}

							@Override
							public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
								return new TALKGUIFIREFORCECAPTAINMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
							}
						}, _bpos);
					}
				}
