...
public class YesecanorProcedure {
				...
				if (entity instanceof ServerPlayer _ent) {
					...
					NetworkHooks.openGui((ServerPlayer) _ent, new MenuProvider() {
						...
						public Component getDisplayName() {
							return new TextComponent("Escanorisee");
						}

						@Override
						public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
							return new EscanoriseeMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
						}
					}, _bpos);
				}
			}
				...
				.orElse(new AnimecrossworkspaceModVariables.PlayerVariables())).goddess == false) {
			{
				if (entity instanceof ServerPlayer _ent) {
					...
					NetworkHooks.openGui((ServerPlayer) _ent, new MenuProvider() {
						@Override
						public Component getDisplayName() {
							return new TextComponent("Escanorteachsunshine");
						}

						@Override
						public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
							return new EscanorteachsunshineMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
						}
					}, _bpos);
				}
			}
