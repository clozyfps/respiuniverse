...
public class BorosSecondFormEntityDiesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (world instanceof ServerLevel _level) {
			Entity entityToSpawn = new BorosThirdFormEntity(AnimecrossworkspaceModEntities.BOROS_THIRD_FORM.get(), _level);
		...
		}
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(new TextComponent((entityiterator.getDisplayName().getString() + ", NO MORE MR NICE GUY!!")),
							(false));
			}
		}
	}
